...
pub struct Ctap1Manager<T: FidoDevice + ?Sized> {
    device_type: PhantomData<T>,
}

#[async_trait]
impl<T> Ctap1<T> for Ctap1Manager<T>
where
    T: FidoDevice + Send,
{
    async fn register(
        ...
        let apdu_response =
            Ctap1Manager::send_apdu_request_wait_uv(device, &apdu_request, request.timeout).await?;
